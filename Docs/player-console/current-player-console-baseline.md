...
更新日期：2026-09-21（状态整理，未重新部署）

最后部署记录为2026-09-09的core1正常固件b53e31f2…；不是早期ded137，也不是因闪烁/错位撤回的core0候选c9b748。输入/头像预览/恢复修复保留；core1对应诊断六项通过，Retarget max57ms仍失败，正常游戏原场景的视觉确认仍待补。

Action17目标灯/自动到达、旋钮每格一步已有用户实机确认；UDP阻断与单站Wi-Fi恢复在此前ad8a版本实测通过。不同版本与场景的证据不能混用。本次整理未重刷或核验当前设备；详见[最新交接](../CURRENT-DEVELOPMENT-HANDOFF.md)及[进度报告](../firmware/player-console-development-progress-2026-09-07.md)。
