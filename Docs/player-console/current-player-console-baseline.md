...
当前工作树包含尚未完成实机验收的候选改动；本页的性能数值是验收要求，不能当作已通过的测量结果。COM7 已部署包含 Action 17 与 Wi-Fi 恢复修正的正常候选（app SHA-256 前缀 ded1375634d2），已验证原房间/席位重连；2026-09-08 本次目标灯光与 Tag 自动到达问题经用户实机确认正常，并有服务端移动观测佐证。动画性能、完整 SelfTest、实际断线恢复和其他边界场景仍待验收。构建、测试及设备状态以 [开发进度记录](../firmware/player-console-development-progress-2026-09-07.md) 为准。
