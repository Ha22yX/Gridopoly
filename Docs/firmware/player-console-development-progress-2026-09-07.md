...
服务器任务随后完成只读有界排查：本机网关可达，旧Pi10.0.0.124 ARP/TCP22/80和health不可达、主机名不能解析，扫描到的19个SSID中无gridopoly；局域网唯一开放SSH的.15主机密钥不匹配Pi，未登录或改变该主机。未发现替代Pi地址，但这些证据不能区分断电、上联丢失或AP服务停止。服务器任务已停止探测并由主任务协调现场供电/上联核对。原场景视觉验收和玩家自动重连观察依赖现场恢复；当前没有后台采集、串口占用或后续重刷计划。收到恢复消息后由玩家任务进行一次有界重连采集，不能将当前无AP当作core1显示修复失败或成功。
