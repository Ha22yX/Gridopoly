...
| `HP_RAW` | U24 后、U201 前的输入；D6/C53 接此处 |
| `HP_RTN` | U201 局部参考/EP，不得与 GND 短接 |
| `24V_BUCK_IN` | U201 受控输出后的本地降压输入 |
...
| `3V3_SYS` | TPS62160DGKR 输出的系统 3.3V |
| `GND` | 全板公共地 |
...
两板当前接口相同：JIN1/JOUT1 的1/2/3为24V_BUS，4为ORDER_IN/OUT，5为BUS_A，6为BUS_B；JIN3/JOUT3的三个脚全为GND。
型号、机械方向和实际镜像核对见[PCB板型](pcb-variants.md)与[互连规范](module-interconnect-5wire.md)。旧五组3Pin磁吸针序不再使用。

## 8. ORDER 顺序检测

```text
JIN1.4 ORDER_IN -> R16 1kΩ -> ESP_ORDER_IN / GPIO9
                                      |
                                      +-> R17 10kΩ -> 3V3_SYS
                                      +-> C24 100nF -> GND
...
EM4100/TK4100 不提供防冲突机制。多个棋子同时位于同一线圈场内时，数据会
叠加并通常导致校验失败或读数不稳定；固件应上报“无有效帧/疑似碰撞”，不得
承诺同时识别多个标签。
...
U23 为 `LMR16030SDDAR`，LCSC `C136648`。

| U23 Pin | 名称 | 当前网络/连接 |
| ---: | --- | --- |
| 1 | BOOT | `BUCK24_BOOT`，C56 100nF 到 SW |
| 2 | VIN | `24V_BUCK_IN` |
| 3 | EN | `HP_RUN`，U201 FLT 与 R206 控制 |
| 4 | RT/SYNC | `BUCK24_RT`，R39 49.9kΩ 到 GND |
| 5 | FB | `BUCK24_FB` |
| 6 | SS | `HP_SS`，C203 2.2µF 到 GND |
| 7 | GND | `GND` |
| 8 | SW | `BUCK24_SW` |
| 9 | EP | `GND` |
...
| D6 | SMBJ30A | Pin1接HP_RAW、Pin2接GND |
| C52/C55 | 2 × 2.2µF/100V | 输入陶瓷电容 |
| C53 | 100nF/100V | HP_RAW侧输入高频去耦 |
| C54 | 47µF/63V | 输入储能 |
| C56 | 100nF | BOOT-SW |
...
| 器件 | 连接/数值 |
| --- | --- |
| R20/R21 | IN1 到 PR1 为 9.1kΩ，PR1 到 GND 为 4.7kΩ |
| R22/R23 | IN2 到 CP2 为 10kΩ，CP2 到 GND 为 4.7kΩ |
| R26/R27 | IN1 到 OV1 为 23.7kΩ，OV1 到 GND 为 5.1kΩ |
| R30/R28 | IN2 到 OV2 为 23.7kΩ，OV2 到 GND 为 5.1kΩ |
| R24 | `PWRMUX_STATUS` 通过 10kΩ上拉到 3.3V |
| R25 | `PWRMUX_STATUS` 到 `PWRMUX_PR1_CFG`，1MΩ |
| R29 | ILM 到 GND，36kΩ |
| C25 | SS 到 GND，100nF |

`PWRMUX_STATUS` 没有连接到 ESP32 GPIO，而是参与 TPS2121 的硬件优先级/
迟滞网络。固件不能直接判断当前由 USB 还是 24V 支路供电。
...
R32、R33 分别将 SDA、SCL 以 4.7kΩ上拉到 `3V3_SYS`。ALERT Pin 3 当前
未连接，固件应轮询测量值。
...
未写 Calibration 寄存器时 Current 和 Power 会保持为零。完整初始化和换算见
[固件开发指南](../firmware/firmware-development-guide.md)。

## 14. 5V 转 3.3V

U25 使用可调输出 `TPS62160DGKR`，当前反馈网络设置为约 3.3V：
...
| 3 EN | `5V_IN`，上电自动使能 |
| 4 AGND | `GND` |
| 5 FB | 反馈中点；R7 312kΩ 到 `3V3_SYS`，R36 100kΩ 到 GND |
| 6 VOS | 直接检测 `3V3_SYS` 输出节点 |
| 7 SW | `BUCK3V3_SW` -> L1 2.2µH -> `3V3_SYS` |
| 8 PG | 当前未连接 |

输入为 C39 10µF 和 C40 100nF；输出为 C2 22µF。反馈公式按
`VOUT = 0.8V * (1 + 312k/100k)` 计算，标称约 3.296V。L1 必须使用满足
TPS62160 峰值电流要求的屏蔽功率电感，不能只按 2.2µH 容值选择普通信号电感。
...
| 位号 | 功能 | 连接 |
| --- | --- | --- |
| SW1 | BOOT | 按下将 `MCU_BOOT`/GPIO0 拉低 |
| SW2 | RESET | 按下将 `MCU_EN` 拉低 |
...
7. 固件启动顺序、默认 GPIO 状态和驱动参数以
   [固件开发指南](../firmware/firmware-development-guide.md)为准。
