...
它不是一张固定印刷的纸板，而是由一块块独立智能格子组成。每个格子都能显示自己的状态、识别实体棋子、驱动灯效，并且通过Wi-Fi/HTTP与游戏中控通信。玩家仍然围在真实桌面前移动棋子，但现金、资产、租金、交易、事件和地图内容都由软件系统实时管理。
...
格子V0.32通过Wi-Fi/HTTP通信并发送物理ORDER信标。双板顺序识别、任一手动锚点重新分配后的跟随、多手动指定保留均已实机验证；RS485为硬件预留，尚未启用。仓库包含36张网页格子图、36张圆屏RGB565格子图和30个头像组件。

截至2026-09-22，棋盘板型为长边PCB2_1、角落PCB2_5。角落使用直接稳压24V输入、5.5×2.1mm中心正极接口，建议24V/5A适配器；USB-PD和NVM配置已取消。两板保留TPS26621支路保护与LMR16030SDDAR降压。整盘60W目标及每板约1W预算仍待实测，适配器120W铭牌不代表PCB额定能力。

下一阶段包括最终PCB布线与DRC、机械配合及ORDER/A/B数据断点、多板供电与温升验证、完整试玩。玩家屏频闪/错位视觉验收仍未关闭，当前core1版Retarget长帧也未达门槛。证据及范围见[当前交接](Docs/CURRENT-DEVELOPMENT-HANDOFF.md)和[硬件报告](Docs/hardware/development-handoff-2026-09-22.md)。
...
| 层级 | 技术 | 用途 |
| --- | --- | --- |
| 格子硬件 | ESP32-S3、ST7789、WS2812、HTRC110、RS485、INA226 | 本地显示、灯效、RFID 识别、Wi-Fi/HTTP通信、预留RS485和电流监测。 |
...
- [头像组件流](Docs/firmware/avatar-component-protocol.md)
- [玩家详情查询](Docs/firmware/player-detail-query-protocol.md)
