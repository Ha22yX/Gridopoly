...
# Project libraries come from this checkout, never an old ToolRoot/libraries copy.
...
$build = if ($ReuseBuildCache) { Join-Path $ToolRoot "build-$Mode" } else { Join-Path $run 'build' }
$output = Join-Path $run 'output'
@{
    mode = $Mode; core = 'esp32:esp32@3.3.11'; source = $project; snapshot = $stage
...
try {
    if ($ReuseBuildCache) {
        # Only one writer can use a mode's cache; other modes remain independent.
        $cacheLock = [IO.File]::Open((Join-Path $ToolRoot "build-$Mode.lock"),
            [IO.FileMode]::OpenOrCreate, [IO.FileAccess]::ReadWrite, [IO.FileShare]::None)
    }
    & $cli compile --fqbn 'esp32:esp32:esp32s3:FlashSize=16M,PartitionScheme=app3M_fat9M_16MB,PSRAM=opi,CDCOnBoot=cdc,DebugLevel=error,UploadMode=default,USBMode=hwcdc' --jobs 8 --build-path $build --output-dir $output --libraries $libraries --config-file $config --build-property "compiler.c.extra_flags=$flags" --build-property "compiler.cpp.extra_flags=$flags" $stage
