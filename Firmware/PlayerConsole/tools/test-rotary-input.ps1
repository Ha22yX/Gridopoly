...
$sources = @('Firmware/PlayerConsole/tools/test-rotary-input.cpp',
...
$arguments += '/Fe:rotary-input-tests.exe'
...
$lines = @('@echo off', "call `"$VsDevCmd`" -arch=x86 >nul", "cd /d `"$OutputDir`"",
    ('cl ' + ($arguments -join ' ')), 'if errorlevel 1 exit /b 1', 'rotary-input-tests.exe')
