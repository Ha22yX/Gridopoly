...
这是格子模块的正式固件工程。当前源码版本为 V0.32 ORDER物理邻接信标（保留 Tag 发布重试）：模块上电后连接树莓派专用热点，
通过HTTP心跳注册、接收显示状态并上报实际ORDER上游。RS485仍未启用。
...
- ORDER_OUT经Q1开漏每6秒发送本机bootId/seq/CRC信标，GPIO10 LOW为释放，HIGH使下游线LOW；GPIO9经现有RC接收实际邻居信标，不使用Wi-Fi注册先后推断线序。
- 独立1ms定时状态机不等待HTTP或显示循环；超过6ms调度间断释放输出并弃帧，接收持续低150ms撤销上游；新有效帧15秒过期，同序号不会刷新证据。完整协议见 [ORDER协议](../../Docs/firmware/tile-order-protocol.md)。
- 心跳保留所有Tag字段，新增order boot/上游boot/seq/age/valid。服务器负责物理图与手动锚点推导；source=order保留在串口，不把旧服务端auto注册分配当物理排序。
- 同一固件按工厂MAC选择显示速率：本机配置的已验证COM6身份使用40MHz，其他模块回8MHz；无屏模块SPI只写不等待像素读回，可正常运行网络/ORDER。
