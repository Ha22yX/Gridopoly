...
这是格子模块的正式固件工程。当前源码版本为 V0.28 Tag 发布重试：模块上电后连接树莓派专用热点，
...
- 从连接页切换到服务器资产页时沿用已工作的 SPI/ST7789 会话直接绘制，并在 1 秒后确认重绘一次；只有显式 REDRAW 命令才重置显示控制器。
- ST7789 正式链路使用 8 MHz SPI；160×160 Flash 素材逐行复制到对齐 RAM 缓冲区后发送，避免 Wi-Fi 活跃时长事务丢写。
